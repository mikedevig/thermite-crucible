...
pub fn run(shared: Arc<SharedState>, num_threads: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    hints.min_size = Some((WIN_W as i32, WIN_H as i32));
    hints.max_size = Some((WIN_W as i32, WIN_H as i32));
...
impl App {
    ...
    fn on_button_press(&mut self, x: i32, y: i32) -> Result<(), Box<dyn std::error::Error>> {
        if BTN_RUN.contains(x, y) && self.state != AppState::SeedChecking {
            ...
            if let Some(ref report) = self.score_report {
                ...
                self.conn.set_selection_owner(self.window, Atom::from(AtomEnum::PRIMARY), x11rb::CURRENT_TIME)?;
    ...
    fn render_maze_to_backbuffer(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        // Clone the snapshot up front so the borrow on `self.last_snapshot`
        // ends here, letting us freely call &mut self methods below.
        let snap = match self.last_snapshot.clone() { Some(s) => s, None => return Ok(()) };
